...
pypl_df = yf.download("PYPL", period="5y")
...
# 2. solver
for s in solvers:
    lr = LogisticRegression(C=best_C, solver=s, max_iter=1000)
    lr.fit(C_X_train, C_y_train)
    wyniki_s.append({
        'C': best_C, 
	'solver': s, 
...
# 3. max_iter
for mi in max_iter:
    lr = LogisticRegression(C=best_C, solver=best_s, max_iter=mi)
    lr.fit(C_X_train, C_y_train)
    wyniki_mi.append({
        'C': best_C, 
	'solver': best_s, 
